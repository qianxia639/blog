...
func (*ArchiveService) GetArchiveGroupByYear() (m map[string][]response.Archive, total int64, err error) {
	...
	sql := `SELECT DATE_FORMAT(updated_at, '%Y') AS year FROM qx_blog GROUP By year ORDER BY year DESC`
	if err = global.QX_DB.Debug().Raw(sql).Scan(&years).Error; err != nil {
		return nil, 0, nil
	}

	for _, year := range years {
		sql := `SELECT id,title,updated_at,flag FROM qx_blog WHERE DATE_FORMAT(updated_at, '%Y') = ?`
